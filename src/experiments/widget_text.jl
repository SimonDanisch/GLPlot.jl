...
function GLPlot.toopengl(style::Style{:Default}, text::String, data::Dict{Symbol, Any})
  ...
  if contains(text, "\n")
    ...
    color_lookup = Texture([
      ...
      rgbaU8(230/256.0, 15/256.0, 80/256.0, 1.0),
...
function edit_text(v0, selection1, unicode_keys)
...
function edit(style::Style{:Default}, textGPU::Texture{GLGlyph{Uint16}, 4, 2}, id::GLushort, custumization::Dict{Symbol, Any})
  ...
  testinput = foldl(edit_text, v00, leftclick_selection, window.inputs[:unicodeinput])
    # selection0 tracks, where the carsor is after a new character addition, selection10 tracks the old selection
end
function addchar(s::Array{GLGlyph{Uint16}, 1}, char::Char, i::Integer)
  if i == 0
    return [glyph, s]
  elseif i == length(s)
    return [s, glyph]
...
function addchar(s::ASCIIString, char::Char, i::Integer)
  char = string(char)
  if i == 0
    return char * s
  elseif i == length(s)
...
function delete(s::ASCIIString, i::Integer)
  if i == 0
    return s
  elseif i == length(s)
    return s[1:end-1]
...
function delete(s::UTF8String, i::Integer)
  if i == 0
    return s
  elseif i == length(s)
    return s[1:end-1]
