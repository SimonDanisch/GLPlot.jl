...
function edit{T <: AbstractArray}(::Style{:Default}, numbertex::Texture{T, 1, 2}, customization::Dict{Symbol,Any})

  backgroundcolor, camera, color, gap, maxdigitis, maxlength, model, window  = values(customization)

  numbers = data(numbertex) # get data from texture/video memory
  text    = Array(GLGlyph{Uint8}, int(size(numbers,1)*maxdigitis), size(numbers, 2))
  ...
  obj         = toopengl(Texture(text);offset=offset, customization...)
  #=
  font        = getfont()
...
-1 8 -1;
-1 -1 -1]
img = glplot(Texture([Vec4(sin(i), cos(j), 0, 1) for i=1:1024, j=1:1024]), kernel=kernel, filternorm=0.1f0, camera=pcamera)
